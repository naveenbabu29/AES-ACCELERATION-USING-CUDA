...
int main() {
    ...
    const int countdown_ms = 500;
    std::promise<float> time_promise;
    std::future<float> time_future = time_promise.get_future();
    bool finished = false;

    auto bomb_timer = std::async(std::launch::async, [&]() {
    ...
    auto cpu_task = std::async(std::launch::async, [&]() {
        ...
        float time_taken = duration_cast<milliseconds>(end - start).count();
        time_promise.set_value(time_taken);

        bool allMatch = true;
        for (int i = 0; i < NUM_BLOCKS; i++) {
            if (memcmp(plaintext[i], decrypted[i], 16) != 0) {
                allMatch = false;
                break;
            }
        }
    ...
    bool correct = cpu_task.get();
    float elapsed = time_future.get();
    cout << "\n[CPU] Encryption + Decryption Time: " << elapsed << " ms\n";
    ascii_bomb(correct && (elapsed <= countdown_ms));
